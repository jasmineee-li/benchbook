...
export function PostCarousel({ onSeeMore }: PostCarouselProps) {
  ...
  const nextPost = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === PREVIEW_POSTS.length - 1 ? 0 : prevIndex + 1
    );
  };

  const prevPost = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? PREVIEW_POSTS.length - 1 : prevIndex - 1
    );
  };
      ...
      <div className="relative">
        <Card className="bg-white p-6 shadow-md rounded-xl min-h-[300px] flex flex-col">
          <div className="text-sm text-amber-700 mb-1">
            {PREVIEW_POSTS[currentIndex].author} •{" "}
            {formatRelativeDate(PREVIEW_POSTS[currentIndex].timestamp)}
          </div>
          <p className="text-amber-900 flex-grow font-serif">
            "{PREVIEW_POSTS[currentIndex].content}"
          </p>
