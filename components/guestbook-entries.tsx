...
export default function GuestbookEntries({ entries }: GuestbookEntriesProps) {
  return (
    <div className="space-y-6 max-w-md mx-auto">
      ...
      {entries.length === 0 ? (
        <p className="text-center text-black">No memories yet. Be the first to leave one!</p>
      ) : (
        entries.map((entry) => (
          <div key={entry.id} className="bg-white p-4 border border-black rounded-xl">
            ...
            <p className="text-sm text-gray-600 mt-2">{new Date(entry.timestamp).toLocaleString()}</p>
          </div>
        ))
      )}
    </div>
